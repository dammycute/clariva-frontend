...
export default function TakeExamPage() {
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      const qs = await api.questions.list({ exam: id }) as Question[];
  ...
  useEffect(() => {
    ...
    const handler = () => {
      ...
      else if (wasHidden) { tabSwitchesRef.current += 1; wasHidden = false; }
    };
  ...
  const timerWarning = displayTime > 0 && displayTime <= 300;
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <span className="text-[10px] text-[#64748B]">
          ...
          {tabSwitchesRef.current > 0 && ` · ${tabSwitchesRef.current} tab switch${tabSwitchesRef.current !== 1 ? 'es' : ''}`}
        </span>
